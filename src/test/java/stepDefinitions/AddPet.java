...
public class AddPet extends TestBase {
    ...
    public void user_hit_the_post_request(DataTable table) throws Throwable {
        ...
        reqSpec = given().spec(requestSpesification())
                .body(data.addpetpayload(testData.get("petType"),testData.get("gender"),testData.get("identifer"), testData.get("breedId"),testData.get("dateOfBirth")));
        response = reqSpec.when().post(URI);
        resSpec = new ResponseSpecBuilder().expectStatusCode(200).expectContentType(ContentType.JSON).build();
    }

  /*  @Then("^verify the status code as \"([^\"]*)\"$")
    public void verify_the_status_code_as_something(String strArg1) throws Throwable {
