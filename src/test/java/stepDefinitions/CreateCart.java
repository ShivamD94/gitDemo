...
import model.Response.quote.CreateQuote.CreateQuoteRes;
import model.Response.quote.CreateQuote.Customer;
...
public class CreateCart extends TestBase {
    ...
    public void user_hit_the_post_cart_request(DataTable table) throws IOException {
        ...
        List<Quote> Quote;
        CreateQuoteRes res = response.getBody().as(CreateQuoteRes.class);
        ...
        Quote=customer.getPets().get(0).getQuotes();
    ...
    public void user_hit_the_get_cart_request(String emailId) throws IOException {
        ...
        if (emailId.equalsIgnoreCase("valid")){
            EmailID=getProperty("ProspectEmail");
        }
    ...
    public void user_validates_the_get_cart_data(){
        ...
        assertEquals(getProperty("ProspectID"),validate.getCustomer().getCustomerId());
        assertEquals(getProperty("ProspectEmail"),validate.getCustomer().getDetails().get(0).getEmail());
        assertEquals(getProperty("PetID"),validate.getCustomer().getPets().get(0).getId());
        assertEquals(quote,validate.getCustomer().getPets().get(0).getQuotes());
        assertEquals(RateMatrix,validate.getCustomer().getPets().get(0).getRateMatrix());
    }
